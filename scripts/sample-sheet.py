...
DATAPATH = Path(__file__).parent.parent.absolute()
os.chdir(DATAPATH)
...
sampleread = [
    os.path.splitext(os.path.basename(fqfiles[i]))[0].split(".")[0]
...
for i in range(0, len(fqfiles)):
    if fqfiles[i].split(".f")[0].endswith("_2"):
        ...
        reads.append("se")
        sampleID.append(sampleread[i].split("_se")[0])


sampledf = pd.DataFrame(
    {"sample_name": [], "fq": [], "full": [], "reads": [], "ext": []}
)
...
filecnt = len(list(os.listdir("./data/")))
filenames = [str(list(os.listdir("./data/"))[i] for i in range(0, filecnt))]
